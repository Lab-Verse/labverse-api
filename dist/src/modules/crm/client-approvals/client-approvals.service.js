"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ClientApprovalsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const client_approval_entity_1 = require("./entities/client-approval.entity");
const security_util_1 = require("../../../common/utils/security.util");
let ClientApprovalsService = class ClientApprovalsService {
    constructor(approvalRepository) {
        this.approvalRepository = approvalRepository;
    }
    async createApproval(createClientApprovalDto) {
        const approval = this.approvalRepository.create(createClientApprovalDto);
        return await this.approvalRepository.save(approval);
    }
    async findAllApprovals() {
        return this.approvalRepository.find();
    }
    async findApprovalsByClient(clientId) {
        return this.approvalRepository.find({ where: { clientId } });
    }
    async findApprovalById(id) {
        const validId = security_util_1.SecurityUtil.validateId(id);
        const approval = await this.approvalRepository.findOne({
            where: { id: validId },
        });
        if (!approval) {
            throw new common_1.NotFoundException(`Client Approval with ID ${id} not found.`);
        }
        return approval;
    }
    async respondToApproval(id, updateClientApprovalDto) {
        const approval = await this.findApprovalById(id);
        if (approval.status !== client_approval_entity_1.ApprovalStatus.PENDING) {
            throw new common_1.NotFoundException('Cannot respond to an approval request that is not pending.');
        }
        approval.respondedAt = new Date();
        this.approvalRepository.merge(approval, updateClientApprovalDto);
        return await this.approvalRepository.save(approval);
    }
    async deleteApproval(id) {
        const validId = security_util_1.SecurityUtil.validateId(id);
        const result = await this.approvalRepository.delete(validId);
        if (result.affected === 0) {
            throw new common_1.NotFoundException(`Client Approval with ID ${id} not found.`);
        }
    }
};
exports.ClientApprovalsService = ClientApprovalsService;
exports.ClientApprovalsService = ClientApprovalsService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(client_approval_entity_1.ClientApproval)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], ClientApprovalsService);
//# sourceMappingURL=client-approvals.service.js.map